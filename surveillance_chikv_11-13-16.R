...
counts1<-counts_numeric2 %>% select(c(54,2,31)) %>% spread(key=ID,value=chikv)
...
summary(population)
chikvcounts_sts<-sts(epoch=counts1$date_numeric,observed= counts1[,2:ncol(counts1)],epochAsDate=FALSE, freq =12, start=c(2014, 10), map=barrios, neighbourhood = barrios$barrios_order, population=population[,2:ncol(population)])
...
##animation###
#animation::saveHTML(animate(chikvcounts_sts, tps=1:19, total.args = list()), title ="Evolution of chikv outbreak in Cali, Colombia, 2014-2016", ani.width = 500, ani.height = 600)
...
barrioID<-as.matrix(counts_numeric2 %>% select(c(54,2,1)) %>% spread(key=ID,value=codigo_barrio))[,-1]
...
jpeg("chikvFit_basic_epiown.jpg")
plotHHH4_maps(chikvFit_basic, which =c("epi.own"), prop=FALSE)
dev.off()

jpeg("chikvFit_basic_neighbors.jpg")
...
chikvFit_offset <-apply(X=SmodelGrid, MARGIN = 1, FUN=function(options){ 
  ...
  "Scovar"=list(f=update(comp$f,~.+ rain*barrioID + temp*barrioID + afro*barrioID + educ*barrioID + empty*barrioID + home*barrioID + limit*barrioID + literate*barrioID + male*barrioID + area*barrioID  + single*barrioID + unemployed*barrioID)))
